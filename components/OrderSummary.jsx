  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      ...
      if (paymentMethod === "RAZORPAY") {
        // data should contain { id, amount, currency, key }
        const { id: orderId, amount, currency: cur, key } = data;

        try {
          await loadRazorpayScript();

          const options = {
            key: key, // RAZORPAY_KEY_ID (public)
            ...
            handler: async function (response) {
              ...
              try {
                // send these to server to verify and mark order paid
                await axios.post(
                  "/api/verify-payment",
                  {
                    ...response,
                    // optionally include orderIds or user id if your verify endpoint needs it
                  },
                  {
                    headers: { Authorization: `Bearer ${token}` },
                  }
                ...
                // clear cart client side & refresh store
                router.push("/orders");
            ...
            notes: {
              // optionally pass more notes here
            },
            theme: { color: "#3399cc" },
          };
          ...
          toast.error(
            typeof err === "string" ? err : "Unable to open payment gateway."
          );
        }
      } else {
        // COD or other non-razorpay (previously stripe) flow
    ...
    <div className="w-full max-w-lg lg:max-w-[340px] bg-slate-50/30 border border-slate-200 text-slate-500 text-sm rounded-xl p-7">
      ...
      <div className="flex justify-between py-4">
        ...
        <p className="font-medium text-right">
          <Protect plan={"plus"} fallback={`${currency}${coupon ? (totalPrice + 0 - (coupon.discount / 100) * totalPrice).toFixed(2) : (totalPrice + 0).toLocaleString()}`}>
            {currency}
